...
public class TestFARC_small {
    ...
    private static void calculate_values(){
        ...
        private static void inizializza(String[] args){
            ...
            //new variables
            NumeroReglas = new double[datasets.size()][algoritmos.size()];
        ...
        public static void generate_DBvector(){
            ...
            cadena = Fichero.myleeFichero("SMALL\\Tablas\\datasets.txt");
            if (cadena.equals("-1")){
		...
		for (int i=0; i<algoritmos.size(); i++) {
			...
			for (int j=0; j<datasets.size(); j++) {
				...
				for (int k=0; k<10; k++) {
					...
					//System.out.println(datasets.get(j));
                                        //System.out.println(cadena);
                                        //System.out.println("SMALL\\resultsFARC\\"+alAct+"."+datasets.get(j)+"\\result"+Integer.toString(k)+"s0e1.txt");
					//System.out.println(cadena);
					lineas = new StringTokenizer (cadena, "\n\r");
					aciertos = total = 0;
					while (lineas.hasMoreTokens()) {
						...
						if ((linea.startsWith("@Number of rules:"))) {
							...
							dato2 = tokens.nextToken();
                                                        ...
                                                        NumeroReglas[j][i]+=Integer.parseInt(dato1);
		...
		cadena = "\n";
		Fichero.escribeFichero("SMALL\\Tablas\\FARC\\TablaFARC.txt", cadena);
		for (int i=0; i<datasets.size(); i++) {
			...
			for (int j=0; j<sel_alg.size(); j++) {
				...
				cadena += String.format("%6.4f", accuracyTst[i][j]) +
                                        "\t"+ String.format("%6.4f", (float)NumeroReglas[i][j] / 10) + 
			...
			cadena += "\n\n\n";	
                ...
                Fichero.AnadirtoFichero("SMALL\\Tablas\\FARC\\TablaFARC.txt", cadena);
        }
        
        public static void write_table_excel() throws IOException, WriteException{
            MyExcelWriter tabla = new MyExcelWriter();
            tabla.setOutputFile("SMALL\\Tablas\\FARC\\ExcelFARC_small.xls");
            ...
            for(int j=0;j<sel_alg.size();j++){
                ...
                for (int i=0; i<datasets.size(); i++) {
                    ...
                    tabla.addNumber(7*j+5,i+2, NumeroReglas[i][j] / 10);
        ...
        private static void writeTable_Bests()throws IOException, WriteException{
           ...
           for (int i=0; i<datasets.size(); i++){
               ...
               for(int j=0;j<sel_alg.size() && elem_dataset_rank[j].getValue()<=ranks_matr[i][0];j++){
                   ...
                   double redSize= 1 - (NumeroReglas[i][index]/NumeroReglas[i][0]);
                   if (redSize>0)
                   ...
                   b=new BigDecimal(NumeroReglas[i][index]/10).setScale(precision,BigDecimal.ROUND_HALF_UP);
		...
		cadena = "";
		Fichero.escribeFichero("SMALL\\Tablas\\FARC\\CSV\\FARC_small_tst.csv", cadena);
                cadena = "Test";
                for(int j=0;j<sel_alg.size();j++){
                cadena += "," + sel_alg.elementAt(j);
                }
                ...
                cadena += "\n";
                Fichero.AnadirtoFichero("SMALL\\Tablas\\FARC\\CSV\\FARC_small_tst.csv", cadena);
        }
        private static void writeCSV_ISRed() throws IOException, WriteException{
		cadena = "";
		Fichero.escribeFichero("SMALL\\Tablas\\FARC\\CSV\\FARC_small_ISRed.csv", cadena);
                cadena = "ISRed";
                for(int j=0;j<sel_alg.size();j++){
                cadena += "," + sel_alg.elementAt(j);
                }
                ...
                cadena += "\n";
                Fichero.AnadirtoFichero("SMALL\\Tablas\\FARC\\CSV\\FARC_small_ISRed.csv", cadena);
        }
        private static void writeCSV_Size() throws IOException, WriteException{
		cadena = "";
		Fichero.escribeFichero("SMALL\\Tablas\\FARC\\CSV\\FARC_small_Size.csv", cadena);
                cadena = "Size";
                for(int j=0;j<sel_alg.size();j++){
                cadena += "," + sel_alg.elementAt(j);
                }
                for (int i=0; i<datasets.size(); i++) {
                    ...
                    for (int j=0; j<algoritmos.size(); j++){
                        //cadena +=","+String.format("%6.4f", accuracyTst[i][j]);
                        cadena +="," +NumeroReglas[i][j]/10;
                    }                       
                }
                cadena += "\n";
                Fichero.AnadirtoFichero("SMALL\\Tablas\\FARC\\CSV\\FARC_small_Size.csv", cadena);
        }
        
        private static void writeCSV_ISRedTst() throws IOException, WriteException{
		cadena = "";
		Fichero.escribeFichero("SMALL\\Tablas\\FARC\\CSV\\FARC_small_ISRedTst.csv", cadena);
                cadena = "ISRedTst";
                for(int j=0;j<sel_alg.size();j++){
                cadena += "," + sel_alg.elementAt(j);
                }
                ...
                cadena += "\n";
                Fichero.AnadirtoFichero("SMALL\\Tablas\\FARC\\CSV\\FARC_small_ISRedTst.csv", cadena);
        }
        private static void writeCSV_RedSizeTst() throws IOException, WriteException{
		cadena = "";
		Fichero.escribeFichero("SMALL\\Tablas\\FARC\\CSV\\FARC_small_RedSizeTst.csv", cadena);
                cadena = "RedSizeTst";
                for(int j=0;j<sel_alg.size();j++){
                cadena += "," + sel_alg.elementAt(j);
                }
                ...
                cadena += "\n";
                Fichero.AnadirtoFichero("SMALL\\Tablas\\FARC\\CSV\\FARC_small_RedSizeTst.csv", cadena);
        }
        ...
        public static void writeExcel_tst() throws IOException, WriteException{
            MyExcelWriter tabla = new MyExcelWriter();
            tabla.setOutputFile("ExcelFARC_small_tst.xls");
            tabla.create("tablaFARC");
            for(int j=0;j<sel_alg.size();j++){
        ...
        */
        ...
        private static void calculteAvgRanks(){
            ...
            double ranks_Size[][]       = Friedmann.calculate_rank(NumeroReglas);
        ...
        private static void calculteAvg(){
            ...
            avg_NumeroReglas = new double[algoritmos.size()];
            ...
            for (int j=0; j<algoritmos.size(); j++) {
                for (int i=0; i<datasets.size(); i++) {
                    ...
                    avg_NumeroReglas[j]  += NumeroReglas[i][j];
                ...
                avg_NumeroReglas[j]  /= datasets.size();
                avg_NumeroReglas[j]  /= 10;
            ...
            elem_AvgNumeroReglas  =ArrayIndex.createArray(avg_NumeroReglas);
            ...
            Arrays.sort(elem_AvgNumeroReglas);
        ...
        private static void calculate_redAVG(){
            for (int i=0; i<datasets.size(); i++) {
                for(int j=0;j<sel_alg.size();j++){
                    RedSize     [i][j]= 1 - (NumeroReglas[i][j]/NumeroReglas[i][0]); 
        ...
        private static void writeTableAvg()throws IOException, WriteException{
            ...
            MyExcelWriter tabla = new MyExcelWriter();
            tabla.setOutputFile("SMALL\\Tablas\\FARC\\ExcelFARC_avg_small.xls");
            tabla.create("tablaFARC");
            ...
            for(int j=0;j<sel_alg.size();j++){
                ...
                // size
                b=new BigDecimal(elem_AvgNumeroReglas[j].getValue()).setScale(precision,BigDecimal.ROUND_HALF_UP);
                tabla.addStringBordL(4,1+j, sel_alg.elementAt(elem_AvgNumeroReglas[j].getIndex()));
                //tabla.addNumber(5,1+j, elem_AvgNumeroReglas[j].getValue());
        ...
        private static void writePareto()throws IOException, WriteException{
            ...
            MyExcelWriter tabla = new MyExcelWriter();
            tabla.setOutputFile("SMALL\\Tablas\\FARC\\Pareto\\ExcelFARC_Pareto_small.xls");
            ...
            for(int j=0;j<sel_alg.size();j++){
                ...
                // size
                b=new BigDecimal(avg_NumeroReglas[elem_AvgTst[j].getIndex()]).setScale(precision,BigDecimal.ROUND_HALF_UP);
        ...
        private static void writeTableAvg_rank()throws IOException, WriteException{
            ...
            MyExcelWriter tabla = new MyExcelWriter();
            tabla.setOutputFile("SMALL\\Tablas\\FARC\\ExcelFARC_Rank_small.xls");
            tabla.create("tablaFARC");
