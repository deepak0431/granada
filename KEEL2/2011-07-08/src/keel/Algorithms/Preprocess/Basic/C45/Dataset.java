...
package keel.Algorithms.Preprocess.Basic.C45;
...
public class Dataset {
    ...
    /** Function to remove all the attributes not selected from the vector.
     *
     * @param selCromosoma	  Boolean vector with the seòected instances
     */
    public final Dataset selectDataset(boolean[] selCromosoma,int numItems) {
        Dataset DatasetSelected = new Dataset(this,numItems);

        for (int i = 0; i < numItemsets(); i++) {
            if (selCromosoma[i]) {
                DatasetSelected.addItemset(itemset(i));
            }
        }
